...
### x & (x - 1)
- x & (x - 1)的效果是将x的为1的visual最低二进制位变为0
...
- map保持元素有序
- unordered_map使用更多内存，内部有一个大数组
- unordered_map遍历时检查所有桶，包括没有元素的桶
- unordered_map适合的操作：插入，删除，搜索

### 移位
- C++根据整型变量是否有符号区分算术移位和逻辑移位
- 逻辑移位：unsigned >>, unsigned <<
- 算术移位：signed >>, signed <<
- 算术左移和逻辑左移等价
- 算术右移高位补符号位，逻辑右移高位补0
- 正数的算术右移和逻辑右移等价，负数的算术右移和逻辑右移分别补1和0
- 负数经过1次逻辑右移后变为正数
- 负数算术右移后仍未负数，直到最终变为-1

### 整数的位运算
- 1 ^ 1 = 0, 0 ^ 0 = 0 ==> 整数的异或可用于去重

### 排列组合
- 组合等价于有重复的排列
- 求从m个元素中选择n个元素的所有组合，等价于求n个1和(m-n)个0的所有排列

### 图论
- 连通：无向图的顶点u和顶点v之间存在路径
- 连通图：无向图的任意两个顶点连通
- 连通分量：无向图的极大连通子图
    - 连通图的连通分量是它本身，只有1个
    - 非连通图有多个连通分量
- 强连通：有向图的顶点u到顶点v和顶点v到顶点u均存在有向路径
- 强连通图：有向图的任意两个顶点强连通
- 强连通分量：有向图的极大强连通子图
    - 强连通图的强连通分量是它本身